export default function WoodTable({ items }) {
  return (
      ...
      <table className="min-w-full bg-white dark:bg-black text-left table-fixed">
        <thead className="bg-neutral-100 dark:bg-neutral-900 border-b border-neutral-200 dark:border-neutral-700 text-sm">
          <tr>
            ...
            <th className="px-6 py-4 font-semibold text-right cursor-pointer select-none w-[20%]">Количество</th>
            <th className="px-6 py-4 font-semibold text-right cursor-pointer select-none w-[20%]">Единица измерения</th>
            <th className="px-6 py-4 font-semibold text-left cursor-pointer select-none w-[40%]">Склад</th>
          </tr>
        </thead>
      </table>
            ...
            {items.length === 0 ? (
              ...
              items.map((item) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-right w-[20%]">{item.quantity}</td>
                  <td className="px-6 py-4 w-[40%]">{item.location}</td>
                </tr>
              ))
            )}
          </tbody>

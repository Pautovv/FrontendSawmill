...
import { FileText, PlusCircle, ChevronRight, ListPlus } from "lucide-react";
import ModalPassport from "./Modals/ModalPassport";
import ModalOperation from "./Modals/ModalOperation";
import ModalWarehouse from "./Modals/ModalWarehouse";
import ModalNomenclature from "./Modals/ModalNomenclature"; // <-- импорт новой модалки
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
      <section className="space-y-4 sm:space-y-6">
        ...
        <div className="rounded-2xl border border-neutral-200/70 dark:border-neutral-800/70 bg-white/60 dark:bg-neutral-900/60 p-4 sm:p-6 shadow-sm">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {actions.map(({ id, title, description, icon: Icon, accent }) => (
              <button
                key={id}
                onClick={() => setModalType(id)}
                className="group relative flex items-start gap-4 rounded-xl border border-neutral-200/70 dark:border-neutral-800/70 bg-white/80 dark:bg-neutral-900/80 p-4 sm:p-5 shadow-sm hover:shadow-lg transition-all hover:-translate-y-0.5 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
              >
